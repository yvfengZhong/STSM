...
def flo(root_path, rerange, para, save, window, overlap):
    x = []
    flag = True
    avg = AverageMeter()

    filename = []
    for line in open(os.path.join(root_path, 'data/dataset/flo_path.txt')):
    ...
    if save:
        data_path = os.path.join(root_path, "flo_" + para[0] + "_" + str(para[1]) + ".txt")
...
if __name__ == "__main__":
    ...
    res = flo(root_path, False, ['laplace', '1e-5'], False, 2, True)
    print(len(res))
    timestats = time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())
    print(timestats)
