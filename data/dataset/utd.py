...
def utd(root_path, rerange, para, save, window, overlap):
    ...
    filename = []
    for line in open(os.path.join(root_path, 'data/dataset/utd_path.txt')):
    ...
    if save:
        data_path = os.path.join(root_path, "utd_" + para[0] + "_" + str(para[1]) + ".txt")
...
if __name__ == "__main__":
    ...
    res = utd(root_path, False, ['laplace', '1e-1'], False, 2, True)
    print(len(res))
    timestats = time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())
    print(timestats)